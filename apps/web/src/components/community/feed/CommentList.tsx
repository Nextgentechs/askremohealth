...
import { type comments } from '@web/server/db/schema'
  ...
  const add = async () => {
    ...
    try {
      ...
      const normalizedComment: CommentWithUser = {
        ...
        content: createdComment.content ?? createdComment.desc ?? '',
    ...
    <>
      {user && (
        <div className="flex items-center gap-4 mt-0">
          <Image
            src={
              doctor?.profilePicture?.url ?? '/assets/community/noAvatar2.png'
            }
            alt=""
            width={32}
            height={32}
          ...
          optimisticComments.map((comment) => (
            <div key={comment.id}>
              <div className="flex gap-4 justify-between mt-6 ml-4">
                ...
                <Image
                  src={
                    comment.profilePicture ?? '/assets/community/noAvatar2.png'
                  }
                  alt=""
                  width={24}
                  height={24}
                      ...
                      user?.id === postAuthorId && (
                        <form
                          action={async () => {
                            try {
                              const result =
                                await initiateConsultMutation.mutateAsync({
